...
#this should have just been guesses <- c(0) because later on, there are problems with indexing the list?
guesses <- list(0)
guesses
...
message('OUT OF BOUNDS. Please try again:')
 ...
 break } }
   ...
   if (length(guesses)-1 == 0){
       next 
    if ((abs((guesses[length(guesses)-1])-rand_int) < (abs (guess-rand_int))) > 10){
      message ("COLDER!")
